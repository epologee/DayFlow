...
- (void)layoutSubviews
{
    ...
    //	We still have DFLDatePickerMonthHeader take a NSDateFormatter formatted title
    ...
    self.imageView.image = [[self class] fetchObjectForKey:[[self class] cacheKeyForPickerDate:self.date] withCreator:^{
        ...
        [[NSString stringWithFormat:@"%lu", (unsigned long)self.date.day] drawInRect:textBounds
                                                            withFont:font
...
+ (id)cacheKeyForPickerDate:(DFLDatePickerDate)date
{
    return @(date.day);
}
